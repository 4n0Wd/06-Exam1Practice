...
Authors: David Mutchler, Vibha Alangar, Valerie Galluzzi, Mark Hays,
         Amanda Stouder, their colleagues and Henry.
...
def problem2b(rect, n, delta, win):
    ...
    rect.attach_to(win)
    for k in range(n - 1):
        corner1 = rg.Point(rect.get_upper_left_corner().x - (k + 1) * delta, rect.get_upper_left_corner().y - (k + 1) * delta)
        corner2 = rg.Point(rect.get_lower_right_corner().x + (k + 1) * delta, rect.get_lower_right_corner().y + (k + 1) * delta)
